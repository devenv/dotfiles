...
4b. **WHO the message appears to come from — verified empirically 2026-08-02.** Despite the env var being named `SLACK_MCP_XOXP_TOKEN`, `~/.claude.json` → `mcpServers.slack.env` currently holds an **xoxb BOT token** (`nils`, U0ACWRM68FJ). Consequences:
   - Anything sent via `mcp__slack__conversations_add_message` **posts as the "nils" bot, not as Boris.** Writing in Boris's first-person voice and then sending it via the tool produces a bot account posting words that read as his — wrong, and confusing to colleagues.
   - Boris's own human↔human DMs are **unreadable** with this token (`conversations.list types=im` returns only bot↔human DMs; `search.messages` → `not_allowed_token_type`).
   - **So:** if the message must genuinely come from Boris, write it in his voice and **hand it to him to paste** (bare URLs, per 4a — literal `<url|text>` shows raw brackets when pasted). Only send via the tool when a bot-attributed post is actually appropriate, and then don't write it as first-person Boris.
   - Boris wants this fixed by swapping that env value for a real personal **xoxp user token** (scopes `im:history`, `im:read`, `search:read`, `mpim:*`) — one change upgrades both sending-identity and DM-reading, for the MCP tools and for core's `ai-instructions/scripts/slack-cr-gather.py` (same key). Re-check with `auth.test` before assuming which regime is live; this is expected to change.

4c. **Never HTML-escape a Slack mention — write the literal `<@USER_ID>`.** Hit this empirically 2026-08-05: composed a message with `&lt;@U04N14WT677&gt;` instead of `<@U04N14WT677>` (auto-escaped while typing the tool-call parameter in a markdown/XML-ish context), and it posted as literal visible text — the mention never rendered as a link and the person was never actually notified. Slack silently accepts and displays the escaped entities instead of erroring, so this fails quiet: the message looks fine in a quick self-check and the miss is only caught by re-reading the posted message's raw text. Always verify a message containing a mention by re-fetching it (`conversations_replies`/`conversations_history`) and checking for literal `&lt;`/`&gt;` before trusting it pinged anyone.
