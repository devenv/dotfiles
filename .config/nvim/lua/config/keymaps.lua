...
function ShowNeotestOutput()
  require("neotest").output.open({ enter = true })
end

function NeotestDebugInfo()
  -- Show neotest status and configuration info
  local neotest = require("neotest")
  print("Neotest status: Running tests with detected adapters")
...
function NeotestRunPattern()
  local pattern = vim.fn.input("Test pattern: ")
  if pattern ~= "" then
    require("neotest").run.run({ extra_args = { "-k", pattern } })
  end
end
...
local mappings = {
  n = {
    ...
    ["<leader>t<tab>"] = { ":lua require('neotest').run.run(vim.fn.getcwd())<CR>", opts = opts },
    ["<leader>e"] = { ":lua require('neotest').output_panel.toggle()<CR>", opts = opts },
    ["<leader>o"] = { ":lua NeotestRunPattern()<CR>", opts = opts },

    -- Navigation for failed tests
    ["]t"] = { ":lua require('neotest').jump.next({ status = 'failed' })<CR>", opts = opts },
    ["[t"] = { ":lua require('neotest').jump.prev({ status = 'failed' })<CR>", opts = opts },

    -- Debug keymaps (updated)
    ["<leader>dt"] = { ":silent! wa<CR>:lua require('neotest').run.run({strategy = 'dap'})<CR>", opts = opts },
